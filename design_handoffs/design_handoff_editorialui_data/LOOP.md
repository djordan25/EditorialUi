# Bundle 8 — Data · /loop instruction
...
Component → spec page:
  EdNativeTable → native-table.html
  EdDataTable → data-table.html
  EdList → list.html
...
Goal: implement EditorialUI Bundle 8 (Data) so each component matches its
design spec EXACTLY. The shipped .tsx/.module.scss are a starting point, not the
finish line.

Oracle (source of truth), in priority order:
  1. The design spec HTML: design_system_v2/components/{native-table,data-table,list}.html
...
EdNativeTable → EdDataTable → EdList
...
- EdDataTable / EdNativeTable stories use a bare `Meta` (no component default).
  Story ids still derive from the export names — verify them in the Storybook sidebar.
- Tables are wide; the demo may exceed the viewport. Bump VIEWPORT in
  visual-verify.mjs if a table is clipped, and prefer diffing a representative
  density/state rather than a giant full table.
