# Bundle 5 — Feedback · /loop instruction
...
Component → spec page:
  EdTooltip → tooltip.html
  EdNotification → notification.html
  EdDialog → dialog.html
  EdEmptyState → empty-state.html
...
Goal: implement EditorialUI Bundle 5 (Feedback) so each component matches its
design spec EXACTLY. The shipped .tsx/.module.scss are a starting point, not the
finish line.

Oracle (source of truth), in priority order:
  1. The design spec HTML: design_system_v2/components/{tooltip,notification,dialog,empty-state}.html
...
EdTooltip → EdNotification → EdDialog → EdEmptyState
...
- EdTooltip and (the portal-rendered) EdDialog mount outside #storybook-root.
  Map their open states with a selector or an inline "always open" story.
- EdDialog lives under EditorialUI/Containers in Storybook — the map reflects that.
