# Bundle 6 — Containers · /loop instruction
...
Component → spec page:
  EdCard → card.html
  EdModal → modal.html
  EdDrawer → side-panel.html
  EdDisclosure → disclosure.html
  EdAccordion → accordion.html
...
Goal: implement EditorialUI Bundle 6 (Containers) so each component matches its
design spec EXACTLY. The shipped .tsx/.module.scss are a starting point, not the
finish line.

Oracle (source of truth), in priority order:
  1. The design spec HTML: design_system_v2/components/{card,modal,side-panel,disclosure,accordion}.html
...
EdCard → EdModal → EdDrawer → EdDisclosure → EdAccordion
...
- EdModal / EdDrawer render into portals and trap focus — use an "open" story
  and map the panel via selector if it's outside #storybook-root.
- EdDrawer's spec page is side-panel.html (the drawer/side-panel anatomy lives there).
