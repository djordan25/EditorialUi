# Bundle 2 — Forms · /loop instruction
...
Component → spec page:
  EdTextField → text-field.html
  EdPasswordInput → password-input.html
  EdCheckbox → checkbox.html
  EdRadioGroup → radio-group.html
  EdSwitch → switch.html
  EdFormControlLabel → form-control-label.html
...
Goal: implement EditorialUI Bundle 2 (Forms) so each component matches its
design spec EXACTLY. The shipped .tsx/.module.scss are a starting point, not the
finish line.

Oracle (source of truth), in priority order:
  1. The design spec HTML: design_system_v2/components/{text-field,password-input,checkbox,radio-group,switch,form-control-label}.html
...
EdTextField → EdPasswordInput → EdCheckbox → EdRadioGroup → EdSwitch → EdFormControlLabel
...
- Form controls are Radix-wrapped — assert on roles/labels, and remember the
  harness drives focus itself, so the focus-ring offset/token is a common diff.
- EdFormControlLabel is a layout primitive; its spec states are mostly anatomy —
  prefer selector-based specs there if labels don't enumerate cleanly.
