# Bundle 7 — Navigation · /loop instruction
...
Component → spec page:
  EdTabs → tabs.html
  EdBreadcrumb → breadcrumb.html
  EdMenu → menu.html
  EdContextMenu → context-menu.html
...
Goal: implement EditorialUI Bundle 7 (Navigation) so each component matches its
design spec EXACTLY. The shipped .tsx/.module.scss are a starting point, not the
finish line.

Oracle (source of truth), in priority order:
  1. The design spec HTML: design_system_v2/components/{tabs,breadcrumb,menu,context-menu}.html
...
EdTabs → EdBreadcrumb → EdMenu → EdContextMenu
...
- EdMenu / EdContextMenu open into portals; map the open menu via selector or an
  inline-open story. EdTabs and EdBreadcrumb diff inline normally.
- EdContextMenu opens on right-click — add a story that renders it open for the diff.
