# Bundle 3 — Selection · /loop instruction
...
Component → spec page:
  EdSelect → select.html
  EdComboBox → combobox.html
  EdAutocomplete → autocomplete.html
...
Goal: implement EditorialUI Bundle 3 (Selection) so each component matches its
design spec EXACTLY. The shipped .tsx/.module.scss are a starting point, not the
finish line.

Oracle (source of truth), in priority order:
  1. The design spec HTML: design_system_v2/components/{select,combobox,autocomplete}.html
...
EdSelect → EdComboBox → EdAutocomplete
...
- EdSelect / EdComboBox / EdAutocomplete render a Radix Popover into a portal.
  The OPEN/menu states won't be inside #storybook-root — either add a story that
  renders the listbox inline, or map those states with a selector against the
  portal. Closed-trigger states diff normally.
